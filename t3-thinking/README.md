...
`seq` starts at zero for each `(threadId, itemId)` and increments after every flush. Deltas are coalesced: a flush goes out once 48 characters are waiting or 120 ms have passed since the last one (checked on the next reasoning event), and whatever is left goes out when the item or turn ends. Every row costs a DB write and a websocket event on each client, so per-delta rows are not worth it; the phone fades each row in.

`itemId` is the provider's reasoning item id when there is one (Codex). Claude Code's thinking deltas carry none, so those are keyed `turn:<turnId>:<segment>`; the segment advances whenever something else starts inside the turn (a tool call, an assistant message, a compaction), which keeps a thinking block that came after a tool call ordered after that tool.

Claude Code announces an automatic context compaction as a status message that the adapter forwards as `session.state.changed` with reason `status:compacting`. The stock server records only the end (`context-compaction` with the token counts, minutes later); the preload appends a `context-compaction` row with `payload.state: "compacting"` and summary `Compacting context` at the start, which the phone shows as the turn's trailing work until the real row lands.
