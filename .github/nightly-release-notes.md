...
**Updating:** later nightlies install from inside the app. Settings → App shows the running commit; the header shows an Update pill when a newer nightly exists. The first in-app update asks Android for the "install unknown apps" permission and a confirmation; after that, updates install silently and a notification offers to reopen the app.
