...
**Install:** download the APK and open it on the phone. It is the *nightly* variant (`com.t3tools.t3code.nightly`, app name "T3 Code Nightly") and installs next to the Play Store app. arm64-v8a only. Signed with the standard Android debug keystore, so each nightly updates the previous one, and it can never replace or be replaced by the store app.
